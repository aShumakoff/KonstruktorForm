...
//  ЭлементРодительВетки	 - Строка - Имя элемента на форме 
//  ДополнительныеПараметры	 Структура - Дополнительные параметры, см. ПолучитьДоппареметрыДляСбораФормы() этого модуля 
//
Процедура СобратьГруппуЭлементовИзXML(Форма,ОбъектДляПостройки,ЭлементРодитель = Неопределено,ЭлементРодительВетки, ДополнительныеПараметры = Неопределено)  Экспорт
	СтруктураЭлементов = СборкаФормПовтИсп.СтруктураЗагружаемыхЭлементовФормы();
	Если ДополнительныеПараметры = Неопределено Тогда
		ДополнительныеПараметры = ПолучитьДоппареметрыДляСбораФормы();
	КонецЕсли;
	...
	Если Команды <> Неопределено и СтроитьПодчиненныеЭлементы Тогда
	...
	//Пока пишу структуру, в планах сделать конструктор на xml описании+
	...
	//
	////Элементы
	//ГруппаПодписьФаксимиле = Форма.Элементы.Найти("ГруппаФаксимильнаяПечать");
	//Если ГруппаПодписьФаксимиле = Неопределено Тогда
	//	
	//	ГруппаПодписьФаксимиле = Форма.Элементы.Добавить("ГруппаФаксимильнаяПечать", 
	//	Тип("ГруппаФормы"));
	//	
	//КонецЕсли;  
	//
...
Функция ПолучитьДоппареметрыДляСбораФормы() Экспорт
	СтруктураПараметров = Новый Структура;
	СтруктураПараметров.Вставить("СтроитьРеквизиты",Ложь);
	СтруктураПараметров.Вставить("СтроитьКоманды",Ложь);
	СтруктураПараметров.Вставить("СтроитьПодчиненныеЭлементы",Ложь);
...
Процедура ПостроитьРеквизитыИзОбъектаXDTO(Форма,ОбъектСОписаниемРеквизитов)
	...
	СборкаФормСервер.СоздатьРеквизиты(Форма,СтрРеквизитов);
...
Процедура ПостроитьПодчиненныеЭлементыИзОбъектаXDTO(Форма,ОбъектСОписаниемЭлементов,ЭлементРодитель)
	...
	СтруктураЭлементов = СборкаФормПовтИсп.СтруктураЗагружаемыхЭлементовФормы().ChildItems;
	СвойстваЭлементов = СтруктураЭлементов.Properties;
	ЗначенияСвойствЭлементов = СтруктураЭлементов.ЗначенияСвойств; 
	...
	Если ТипЗНЧ(ТипДляразбора) = ТИП("ОбъектXDTO") Тогда
		...
		Если ТипДляразбора.Type = "xs:string" Тогда
			...
			Если ТипДляразбора.свойства().получить("StringQualifiers")<>неопределено Тогда
				...
				ОписаниеТипа = СборкаФормСервер.ОписаниеТипаСтрока(Длина);
				
			КонецЕсли;
			
		ИначеЕсли ТипДляразбора.Type = "xs:boolean" Тогда	
			
			ОписаниеТипа = СборкаФормСервер.ОписаниеТипаБулево();
